...
export default function MyNewsPage() {
  ...
  const [searchDate, setSearchDate] = useState<{ day: string; month: string; year: string }>({
    day: '',
    month: '',
    year: '',
  })
  ...
  const fetchNews = async (userId: string) => {
    try {
      ...
      if (data) {
        setNews(data)
        const statsMap = data.reduce((acc: Record<string, any>, item: any) => {
          acc[item.id] = {
  ...
  const clearSearch = () => {
    ...
    setSearchTitle('')
    fetchNews(session?.user?.id) // Re-fetch all news
  }
